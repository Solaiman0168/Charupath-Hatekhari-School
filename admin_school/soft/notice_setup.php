...
<?php
    ...
    if(isset($_POST['submit']))
    {
           ...
           $SQL = "SELECT * FROM $tbl_name WHERE `is_deleted` = 0 AND UPPER (`bn_title`) = '".strtoupper($bn_title)."' AND `date`='$date' ";
            ...
            if($COUNT < 1):
               ...
               echo  $sql = "INSERT INTO $tbl_name SET `bn_title` = '$bn_title', `date` = '$date',`bn_image_url` = '$bn_image_url',  `created_by`='$user_no', `created_on`='$CURR_TIME'";
    ...
    if(isset($_POST['update']))
    {
            ...
            $date = trim($_POST['date']);
           $SQL = "SELECT * FROM $tbl_name WHERE `is_deleted` = 0 AND UPPER (`bn_title`) = '".strtoupper($bn_title)."' AND `date`='$date' AND `notice_id` != '$notice_id'";
            $COUNT = mysqli_num_rows(mysqli_query($con,$SQL));
            if($COUNT < 1): 
                if ($_FILES["bn_image_url"]["error"] > 0) {
                    $bn_image_url = "No Notice Uploaded";
             ...
             $sql = "UPDATE $tbl_name SET `bn_title` = '$bn_title' , `date` = '$date' ,`bn_image_url` = '$bn_image_url' ,`is_updated` = 1, `updated_by` = $user_no ,`updated_on` = '$CURR_TIME' WHERE notice_id = '$notice_id'";
                $result = mysqli_query($con,$sql);
                if($result)
                {
...
?>
    ...
    ?>
     ...
     <div class="form-group " <?php if($mgs=='')echo "style='display:none;'" ?>>
        ...
        <div class="form-group ">
            <label for="date" class="control-label col-lg-2"> তারিখ </label>
            <div class="col-lg-8">
                <input class=" form-control"  id="" name="date" type="date" value="<?=$result['date']?>" required />
            </div>
        </div>
        <div class="form-group "> 
    ...
    ?>
      <form class="cmxform form-horizontal " id="signupForm" method="post" action="" enctype="multipart/form-data">
        ...
        <div class="form-group ">
            <label for="date" class="control-label col-lg-2"> তারিখ </label>
            <div class="col-lg-8">
                <input  id=""  class=" form-control" name="date" type="date" required />
            </div>
        </div>
        <div class="form-group "> 
            <label for="bn_image_url" class="control-label col-lg-2"> আপলোড </label>
...
?>
    ...
    <table   class="table table-bordered table-hover dataTable col-xs-12 col-sm-12 col-md-4 col-lg-8 ">
        <tr>
           <th>নং</th>
            <th>টাইটেল</th>
             <th>তারিখ</th>
