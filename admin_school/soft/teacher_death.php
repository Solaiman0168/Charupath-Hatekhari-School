...
<?php $table_heading = "মৃত শিক্ষক/শিক্ষিকা  ";?>
<?php include 'include/sidebar.php';?>
<?php include 'include/body-top.php';?>
  
<?php
        $tbl_name="teacher_death";       //your table name
        $targetpage = "teacher_death.php";   //your file name  (the name of this file)
    ...
    {
        $ID = $_GET['delete'];
        $sql = "UPDATE $tbl_name SET `is_deleted` = 1 ,`deleted_by` = '$user_no', `deleted_on` = '$CURR_TIME' WHERE teacher_death_id = $ID";
    ...
    if(isset($_POST['update']))
    {
            ...
            $bn_name = trim($_POST['bn_name']);
            $en_name = trim($_POST['en_name']);
            $teacher_death_id = $_POST['teacher_death_id'];
            $bn_designation =trim( $_POST['bn_designation']);
            $en_designation = trim($_POST['en_designation']);
            $bn_mobile =$_POST['bn_mobile'];
            $en_mobile =$_POST['en_mobile'];
        ...
        $SQL = "SELECT * FROM $tbl_name WHERE `is_deleted` = 0 AND `bn_name` ='$bn_name' AND `en_name` = '$en_name' AND `bn_designation` = '$bn_designation' AND `en_designation` = '$en_designation'AND `bn_mobile` = '$bn_mobile'AND `en_mobile` = '$en_mobile'AND `email` = '$email'AND `bn_degree` = '$bn_degree' AND `en_degree` = '$en_degree'AND `bn_dob` ='$bn_dob' AND `mpo_no`='$mpo_no' AND `bn_job_duration` = '$bn_job_duration'AND `bn_present_address` = '$bn_present_address'AND `en_present_address` = '$en_present_address'AND `bn_permanent_address` = '$bn_permanent_address'AND `en_permanent_address` = '$en_permanent_address'AND `bn_speech_title` = '$bn_speech_title'AND `en_speech_title` = '$en_speech_title'AND `bn_speech` = '$bn_speech'AND `en_speech` = '$en_speech' AND `teacher_death_id` != '$teacher_death_id'";
...
?>
    ...
    <?php
        if(isset($_GET['edit'])):
        $id = $_GET['edit'];
        $sql = "SELECT * FROM $tbl_name WHERE `teacher_death_id` = '$id' ";
    ...
    ?>
     ...
     <div class="form-group " <?php if($mgs=='')echo "style='display:none;'" ?>>
            ...
            <div>
                <input type="hidden" name="teacher_death_id" value="<?=$result['teacher_death_id']?>" />
            </div>
        </div>
...
<div style="overflow: auto">
    ...
    <?php $i=$page*$limit-$limit+1; while($row = mysqli_fetch_array($result)):?>
        <tr>
            ...
            <td>
                <a onclick="return confirm('Are you Sure Want to Edit?');" href="<?=$targetpage.'?edit='.$row['teacher_death_id']?>" class="btn btn-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                <a onclick="return confirm('Are you Sure Want to Delete?');" href="<?=$targetpage.'?delete='.$row['teacher_death_id']?>" class="btn btn-danger"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
            </td>
        </tr>
    <?php endwhile;?>
    </table>
