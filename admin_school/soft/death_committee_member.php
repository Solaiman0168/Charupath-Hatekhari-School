...
<?php $table_heading = "যারা ইন্তেকাল করেছেন";?>
<?php include 'include/sidebar.php';?>
<?php include 'include/body-top.php';?>
  
<?php
        $tbl_name="death_committee_member";       //your table name
        $targetpage = "death_committee_member.php";   //your file name  (the name of this file)
    ...
    {
        $ID = $_GET['delete'];
        $sql = "UPDATE $tbl_name SET `is_deleted` = 1 ,`deleted_by` = '$user_no', `deleted_on` = '$CURR_TIME' WHERE death_committee_member_id = $ID";
    ...
    if(isset($_POST['update']))
    {
            ...
            $bn_name = trim($_POST['bn_name']);
            $en_name = trim($_POST['en_name']);
            $death_committee_member_id = $_POST['death_committee_member_id'];
            $bn_designation =trim( $_POST['bn_designation']);
            $en_designation = trim($_POST['en_designation']);
            $bn_mobile =$_POST['bn_mobile'];
            $en_mobile =$_POST['en_mobile'];
        ...
        $SQL = "SELECT * FROM $tbl_name WHERE `is_deleted` = 0 AND `bn_name` ='$bn_name' AND `en_name` = '$en_name' AND `bn_designation` = '$bn_designation' AND `en_designation` = '$en_designation'AND `bn_mobile` = '$bn_mobile'AND `en_mobile` = '$en_mobile'AND `email` = '$email'AND `bn_degree` = '$bn_degree' AND `en_degree` = '$en_degree'AND `bn_dob` ='$bn_dob' AND `en_dob`='$en_dob' AND `bn_job_duration` = '$bn_job_duration'AND `en_job_duration` = '$en_job_duration'AND `bn_present_address` = '$bn_present_address'AND `en_present_address` = '$en_present_address'AND `bn_permanent_address` = '$bn_permanent_address'AND `en_permanent_address` = '$en_permanent_address'AND `bn_speech_title` = '$bn_speech_title'AND `en_speech_title` = '$en_speech_title'AND `bn_speech` = '$bn_speech'AND `en_speech` = '$en_speech' AND `death_committee_member_id` != '$death_committee_member_id'";
...
?>
    ...
    <?php
        if(isset($_GET['edit'])):
        $id = $_GET['edit'];
        $sql = "SELECT * FROM $tbl_name WHERE `death_committee_member_id` = '$id' ";
    ...
    ?>
     ...
     <div class="form-group " <?php if($mgs=='')echo "style='display:none;'" ?>>
            ...
            <div>
                <input type="hidden" name="death_committee_member_id" value="<?=$result['death_committee_member_id']?>" />
            </div>
        </div>
      ...
      <div class="form-group ">
        ...
        <div class="form-group ">
            <label for="bn_permanent_address" class="control-label col-lg-2">স্থায়ী ঠিকানা  </label>
            <div class="col-lg-4">
    ...
    <form class="cmxform form-horizontal " id="signupForm" method="post" action="" enctype="multipart/form-data">
       ...
       <div class="form-group ">
        ...
        <div class="form-group ">
            ...
            <div class="col-lg-4">
                <input class=" form-control" id="" name="bn_mobile" type="text"  />
            </div>
            <label for="en_mobile" class="control-label col-lg-2"> Mobile
            </label>
            <div class="col-lg-4">
                <input class=" form-control" id="" name="en_mobile" type="text" />
            </div>
        </div>
        <div class="form-group ">
...
<div style="overflow: auto">
    ...
    <?php $i=$page*$limit-$limit+1; while($row = mysqli_fetch_array($result)):?>
        <tr>
            ...
            <td>
                <a onclick="return confirm('Are you Sure Want to Edit?');" href="<?=$targetpage.'?edit='.$row['death_committee_member_id']?>" class="btn btn-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                <a onclick="return confirm('Are you Sure Want to Delete?');" href="<?=$targetpage.'?delete='.$row['death_committee_member_id']?>" class="btn btn-danger"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
            </td>
        </tr>
    <?php endwhile;?>
    </table>
