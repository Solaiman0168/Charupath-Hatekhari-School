...
<?php $table_heading = "Semester Suggestion English Version";?>
<?php include 'include/sidebar.php';?>
<?php include 'include/body-top.php';?>
   
<?php
        $tbl_name="suggestion_term_exam_english";       //your table name
        $targetpage = "english_v_sem_sugg.php";   //your file name  (the name of this file)
    ...
    {
        $ID = $_GET['delete'];
        $sql = "UPDATE $tbl_name SET `is_deleted` = 1 ,`deleted_by` = '$user_no', `deleted_on` = '$CURR_TIME' WHERE suggestion_term_exam_no = $ID";
    ...
    if(isset($_POST['submit']))
    {
           ...
           $SQL = "SELECT * FROM $tbl_name WHERE `is_deleted` = 0 AND UPPER (`title`) = '".strtoupper($title)."'";
            ...
            if($COUNT < 1):
                if ($_FILES["image_url"]["error"] > 0) {
                 ...
                 $sql = "INSERT INTO $tbl_name SET `title` = '$title',`image_url` = '$image_url',  `created_by`='$user_no', `created_on`='$CURR_TIME'";
...
?>
     ...
     <form class="cmxform form-horizontal " id="signupForm" method="post" action="" enctype="multipart/form-data">
        ...
        <div class="form-group ">
            <label for="title" class="control-label col-lg-3">টাইটেল </label>
            <div class="col-lg-4">
                <input class=" form-control" id="" name="title" type="text" required />
            </div>
        </div>
        <div class="form-group ">
            <label for="title" class="control-label col-lg-3">সাজেশন</label>
            <div class="col-lg-4">
...
?>
    ...
    <?php $i=$page*$limit-$limit+1; while($row = mysqli_fetch_array($result)):?>
        <tr>
            ...
            <td>
                <a onclick="return confirm('Are you Sure Want to Delete?');" href="<?=$targetpage.'?delete='.$row['suggestion_term_exam_no']?>" class="btn btn-danger"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                </td>
        </tr>
    <?php endwhile;?>
    </table>
