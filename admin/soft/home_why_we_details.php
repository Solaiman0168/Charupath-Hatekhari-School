...
<?php $table_heading = "যে কারণে আপনার সন্তানকে চারুপাঠ হাতেখড়ি স্কুলে ভর্তি করবেন";?>
...
<?php
    ...
    if(isset($_POST['submit']))
    {
         ...
         $SQL = "SELECT * FROM $tbl_name WHERE  `is_deleted` = 0 AND `bn_details` = '$bn_details' ";
            $COUNT = mysqli_num_rows(mysqli_query($con,$SQL));
            if($COUNT < 1):
                 $sql = "INSERT INTO $tbl_name (`bn_details` ,`created_by` , `created_on`) VALUES('$bn_details','$en_details', $user_no, '$CURR_TIME')";
                $result = mysqli_query($con,$sql);
                if($result)
                {
    ...
    if(isset($_POST['update']))
    {
           ...
           $SQL = "SELECT * FROM $tbl_name WHERE `is_deleted` = 0  AND UPPER (`bn_details`) = '".strtoupper($bn_details)."'  AND `home_why_we_id` != '$home_why_we_id'";
            $COUNT = mysqli_num_rows(mysqli_query($con,$SQL));
            if($COUNT < 1): 
             $sql = "UPDATE $tbl_name SET `bn_details` = '$bn_details' ,`is_updated` = 1, `updated_by` = $user_no ,`updated_on` = '$CURR_TIME' WHERE home_why_we_id = '$home_why_we_id'";
                $result = mysqli_query($con,$sql);
                if($result)
                {
    ...
    ?>
     ...
     <div class="form-group " <?php if($mgs=='')echo "style='display:none;'" ?>>
        ...
        <div class="form-group ">
            <label for="bn_details" class="control-label col-lg-2">কারন </label>
            <div class="col-lg-8">
